  ...
  const cancelPacking = () => {
    ...
    if (id.endsWith('/src/packing/packer.ts')) {
      ...
      if (!code.includes('async function fastUniformPackAsync(')) {
        ...
        if (fastStart !== -1) {
          const asyncFn = `async function fastUniformPackAsync(state:ReturnType<typeof buildState>,c:Container,step?:(i:number,n:number)=>void,signal?:AbortSignal){
...
`
      ...
      code = code.replace("if(fastUniformPack(state,c,report)){progress?.(100);return state.items}", "if(await fastUniformPackAsync(state,c,report,signal)){progress?.(100);return state.items}")
