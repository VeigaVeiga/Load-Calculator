...
function candidatePoints(items:PlacedCargo[],c:Container):Point[]{
  ...
  const add=(x:number,y:number,z:number)=>{x=clean(x);y=clean(y);z=clean(z);if(x<-EPS||y<-EPS||z<-EPS||x>c.length+EPS||y>c.width+EPS||z>c.height+EPS)return;const k=key(x,y,z);if(!seen.has(k)){seen.add(k);out.push({x,y,z})}}
  add(0,0,0)
  const xs=new Set<number>([0,c.length]),ys=new Set<number>([0,c.width]),zs=new Set<number>([0])
  for(const q of items){const d=dims(q);add(q.x+d.length,q.y,q.z);add(q.x,q.y+d.width,q.z);add(q.x+d.length,q.y+d.width,q.z);add(q.x,q.y,q.z+q.height);xs.add(q.x);xs.add(q.x+d.length);ys.add(q.y);ys.add(q.y+d.width);zs.add(q.z);zs.add(q.z+q.height)}
  const xu=[...xs].sort((a,b)=>a-b),yu=[...ys].sort((a,b)=>a-b),zu=[...zs].sort((a,b)=>a-b)
  for(const z of zu)for(const x of xu)for(const y of yu)add(x,y,z)
...
function contactScore(p:PlacedCargo,items:PlacedCargo[],c:Container){
  const d=dims(p),area=d.length*d.width
  let side=0,top=0
  for(const q of items){const qd=dims(q),a=rectArea(p,q);if(a<=EPS)continue;if(Math.abs(p.z-(q.z+q.height))<=EPS)top+=a/area;if(Math.abs(p.x-(q.x+qd.length))<=EPS||Math.abs(p.x+d.length-q.x)<=EPS)side+=Math.min(d.width,qd.width)/Math.max(d.width,qd.width);if(Math.abs(p.y-(q.y+qd.width))<=EPS||Math.abs(p.y+d.width-q.y)<=EPS)side+=Math.min(d.length,qd.length)/Math.max(d.length,qd.length)}
  const wall=(p.x<=EPS?1:0)+(Math.abs(p.x+d.length-c.length)<=EPS?1:0)+(p.y<=EPS?1:0)+(Math.abs(p.y+d.width-c.width)<=EPS?1:0)
  return p.z*100000+(1-Math.min(1,top))*1200+(1-Math.min(1,side/4))*300+(c.length-(p.x+d.length))*0.02+(c.width-(p.y+d.width))*0.02-wall*80-(d.length*d.width*p.height)*0.000001
}
...
function orderUnits(cargo:Cargo[],strategy:'footprint'|'height'|'volume'|'weight'='footprint'){
  return expandCargo(cargo).filter(u=>u.cargo.length>0&&u.cargo.width>0&&u.cargo.height>0).sort((a,b)=>{
    const av=a.cargo.length*a.cargo.width*a.cargo.height,bv=b.cargo.length*b.cargo.width*b.cargo.height,af=a.cargo.length*a.cargo.width,bf=b.cargo.length*b.cargo.width
...
function packCore(cargo:Cargo[],c:Container,locked:PlacedCargo[],strategy:'footprint'|'height'|'volume'|'weight',onStep?:(i:number,total:number)=>void){
  ...
  for(let i=0;i<units.length;i++){
    const u=units[i],points=candidatePoints(items,c);let best:PlacedCargo|undefined,bestScore=Infinity
    for(const o of orientations(u.cargo)){
      ...
      for(const pt of points){
        ...
        const score=contactScore(p,items,c);if(score<bestScore){bestScore=score;best=p}
      }
    }
...
export function autoPack(cargo:Cargo[],c:Container,locked:PlacedCargo[]=[]){
  ...
  let best:PlacedCargo[]=[];let bestScore=-Infinity
  for(const s of strategies){const r=packCore(cargo,c,locked,s);const sc=solutionScore(r,c,total);if(sc>bestScore){bestScore=sc;best=r}if(r.length>=total)return r}
  return best
}
...
export async function autoPackAsync(cargo:Cargo[],c:Container,locked:PlacedCargo[]=[],onProgress?:(percent:number)=>void){
  ...
  const strategies:['footprint','height','volume']=['footprint','height','volume']
  let best:PlacedCargo[]=[];let bestScore=-Infinity
  for(let si=0;si<strategies.length;si++){
    const strategy=strategies[si]
    const result=await packAsyncStrategy(cargo,c,locked,strategy,p=>onProgress?.(Math.min(99,Math.round((si*100+p)/strategies.length))))
...
async function packAsyncStrategy(cargo:Cargo[],c:Container,locked:PlacedCargo[],strategy:'footprint'|'height'|'volume',onProgress?:(p:number)=>void){
  ...
  for(const p of items)lockedCount.set(p.cargoId,(lockedCount.get(p.cargoId)||0)+1)
  const units=orderUnits(cargo,strategy).filter(u=>u.index>=(lockedCount.get(u.cargo.id)||0));let totalWeight=items.reduce((s,p)=>s+p.weight,0)
  for(let i=0;i<units.length;i++){
    const u=units[i],points=candidatePoints(items,c);let best:PlacedCargo|undefined,bestScore=Infinity
    for(const o of orientations(u.cargo)){
      if(o.length>c.length+EPS||o.width>c.width+EPS||u.cargo.height>c.height+EPS)continue
      for(const pt of points){
        ...
        const p=makePlaced(u,o,pt.x,pt.y,pt.z)
        if(totalWeight+p.weight>c.maxPayload+EPS||collision(p,items)||!canStack(p,u.cargo,items,defs))continue
        const sc=contactScore(p,items,c);if(sc<bestScore){bestScore=sc;best=p}
      }
    }
    if(best){items.push(best);totalWeight+=best.weight}
    onProgress?.(units.length?Math.round(((i+1)/units.length)*100):100)
