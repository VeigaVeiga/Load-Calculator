...
function supportFor(
  ...
  for (const q of placed) {
    ...
    supportedArea += area
    maxDepth = Math.max(maxDepth, q.stackDepth + 1)
    palletSupport ||= q.p.cargoType === 'pallet'
    supports.push(q)
  }

  if (supportedArea < baseArea * 0.98) return { ok: false, depth: 0, palletSupport }
  ...
  if (cargo.loadBearing && cargo.stackable && configured <= 0) {
    // Unlimited stacking is allowed, but the container height is still enforced
    // by fitsContainer().
  }

  for (const q of supports) {
    const limit = Number.isFinite(cargoById.get(q.p.cargoId)?.maxLoadOnTop)
      ? cargoById.get(q.p.cargoId)?.maxLoadOnTop ?? 0
      : 0
    if (limit > 0 && (supportLoads.get(q.p.id) ?? 0) + cargo.weight > limit + EPS) return { ok: false, depth: maxDepth, palletSupport }
  }
...
function candidateScore(
  ...
  if (support.palletSupport) score -= 50000000
  return score
}

function makePlaced(c: Cargo, index: number, o: { length: number; width: number; rotation: 0 | 90 }, x: number, y: number, z: number): PlacedCargo {
  return {
    id: `${c.id}-${index + 1}`,
    cargoId: c.id,
    cargoType: c.type,
    x, y, z,
    // Keep the user's original dimensions here. Rotation is applied once by
    // geometry/rendering; this prevents the historical double-rotation bug.
    length: c.length,
    width: c.width,
    height: c.height,
    weight: c.weight,
...
export async function autoPackExtreme(
  ...
  for (const group of groups) {
    ...
    while (remaining > 0) {
      ...
      let best: { point: CandidatePoint; orientation: { length: number; width: number; rotation: 0 | 90 }; score: number; depth: number; palletSupport: boolean } | undefined
      ...
      for (const point of points) {
        for (const o of orientations(c)) {
          ...
          const score = candidateScore(c, point, o, container, support)
          if (!best || score < best.score) best = { point, orientation: o, score, depth: support.depth, palletSupport: support.palletSupport }
        }
      }
