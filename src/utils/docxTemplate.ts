...
export async function extractZipEntry(bytes:Uint8Array,name:string){
  ...
  // BlobPart in newer TypeScript lib.dom requires an ArrayBuffer rather than
  // ArrayBufferLike. Copy the exact bytes into a fresh ArrayBuffer.
...
export async function createDocxWithPlanImages(template:ArrayBuffer,xmlText:string,topPng:Uint8Array,sideFrontPng:Uint8Array){
  ...
  const w='http://schemas.openxmlformats.org/wordprocessingml/2006/main',r='http://schemas.openxmlformats.org/officeDocument/2006/relationships',tables=Array.from(doc.getElementsByTagNameNS(w,'tbl')),plan=tables[1]
