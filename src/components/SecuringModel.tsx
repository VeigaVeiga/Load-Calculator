...
export default function SecuringModel({item,container,selected,onSelect,tool='move'}:{item:SecuringItem;container:Container;selected:boolean;onSelect:()=>void;onMove:(id:string,x:number,y:number)=>void;onRotate:(id:string,rotation:number)=>void;onScale:(id:string,factor:number)=>void;onDragState:(v:boolean)=>void;airBagStretch?:boolean;focusMode?:boolean;tool?:SceneTool}){
 ...
 return <group position={center} rotation={[0,0,item.rotation*Math.PI/180]} scale={[scale,scale,scale]} onPointerDown={e=>{e.stopPropagation();onSelect()}} onClick={e=>{e.stopPropagation();onSelect()}}>
