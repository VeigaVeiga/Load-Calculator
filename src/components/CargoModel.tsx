...
function CargoModel({ p, container, selected, onPointerDown, onPointerUp, onPointerMove, onClick, showName = false, local = false, hovered = false }: CargoModelProps) {
  // The parent CargoObject rotates the complete model around Z. Therefore the
  // geometry itself must always use the unrotated physical L/W dimensions.
  // Previously dims(p) was combined with the parent's rotation, producing a
  // second 90° swap and making rotated cartons appear to overlap.
  const d = dims(p)
