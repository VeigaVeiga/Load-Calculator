...
function CargoModel({p,container,selected,onPointerDown,onPointerUp,onPointerMove,onClick,showName=false,local=false,hovered=false}:{p:PlacedCargo;container:Container;selected:boolean;onPointerDown:(e:any)=>void;onPointerUp:(e:any)=>void;onPointerMove:(e:any)=>void;onClick:(e:any)=>void;showName?:boolean;local?:boolean;hovered?:boolean}){
 ...
 const dl=Math.max(40,d.length-GAP*2),dw=Math.max(40,d.width-GAP*2)
 const baseH=Math.min(130,Math.max(80,p.height*.10)),bodyH=Math.max(40,p.height-baseH)
 return <group position={pos} rotation={[0,0,local?0:p.rotation*Math.PI/180]}>
  <mesh userData={{collisionBody:false,hitBox:true,cargoId:p.id}} onPointerDown={onPointerDown} onPointerMove={onPointerMove} onPointerUp={onPointerUp} onClick={onClick}>
   <boxGeometry args={[Math.max(d.length,30)*S,Math.max(d.width,30)*S,Math.max(p.height,30)*S]}/><meshBasicMaterial transparent opacity={0} depthWrite={false}/>
  </mesh>
  {pallet?<>
   <mesh position={[0,0,-p.height*S/2+baseH*S/2]}><boxGeometry args={[dl*S,dw*S,baseH*S]}/><meshStandardMaterial color="#8a5a32" roughness={.86}/></mesh>
   <mesh position={[0,0,-p.height*S/2+baseH*S*.72]}><boxGeometry args={[dl*S,dw*S*.18,baseH*S*.22]}/><meshStandardMaterial color="#b57b45" roughness={.86}/></mesh>
   <mesh position={[0,0,-p.height*S/2+baseH*S*.72]}><boxGeometry args={[dl*S*.18,dw*S,baseH*S*.22]}/><meshStandardMaterial color="#b57b45" roughness={.86}/></mesh>
   ...
   <mesh position={[0,0,bodyH*S/2+.002]}><boxGeometry args={[Math.min(dl*.18,180)*S,dw*S+.01,.018]}/><meshStandardMaterial color="#d6bd82" roughness={.8}/></mesh>
   <mesh position={[0,-dw*S/2-.003,0]}><boxGeometry args={[dl*S,.014,bodyH*S*.72]}/><meshStandardMaterial color="#8e6b4e" roughness={.8}/></mesh>
  </>}
