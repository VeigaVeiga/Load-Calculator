...
function CargoModel({p,container,selected,onPointerDown,onPointerUp,onPointerMove,onClick,showName=false,local=false,hovered=false}:{p:PlacedCargo;container:Container;selected:boolean;onPointerDown:(e:any)=>void;onPointerUp:(e:any)=>void;onPointerMove:(e:any)=>void;onClick:(e:any)=>void;showName?:boolean;local?:boolean;hovered?:boolean}){
 ...
 const forkGap=Math.max(120,Math.min(260,d.width*.16))
 const beamW=Math.max(70,(dw-forkGap*2)/3)
 return <group position={pos} rotation={[0,0,local?0:p.rotation*Math.PI/180]}>
  ...
  {pallet?<>
    <group position={[0,0,-p.height*S/2+baseH*S/2]}>
      {[-1,0,1].map(i=><mesh key={`beam-${i}`} position={[0,i*(beamW+forkGap),0]}><boxGeometry args={[dl*S,beamW*S,baseH*S]}/><meshStandardMaterial color="#8a5a32" roughness={.86}/></mesh>)}
      <mesh position={[0,0,baseH*S*.38]}><boxGeometry args={[dl*S,dw*S,baseH*S*.20]}/><meshStandardMaterial color="#b57b45" roughness={.86}/></mesh>
    </group>
    ...
    <mesh position={[0,-dw*S/2-.004,0]}><boxGeometry args={[dl*S,.018,bodyH*S*.72]}/><meshStandardMaterial color="#6e4a2d" roughness={.9}/></mesh>
  </>:<>
    ...
    <mesh position={[0,dw*S/2+.003,0]}><boxGeometry args={[dl*S,.014,bodyH*S*.72]}/><meshStandardMaterial color="#8e6b4e" roughness={.8}/></mesh>
    <mesh position={[-dl*S/2-.003,0,0]}><boxGeometry args={[.014,dw*S,bodyH*S*.72]}/><meshStandardMaterial color="#8e6b4e" roughness={.8}/></mesh>
  </>}
