...
function CargoObject({ p, container, selected, onSelect, registerRef, showName }: { p: PlacedCargo; container: Container; selected: boolean; onSelect: () => void; registerRef: (id: string, o: THREE.Group | null) => void; showName: boolean }) {
  ...
  useLayoutEffect(() => { const o = ref.current; if (!o) return; const d = dims(p); o.position.set((p.x+d.length/2-container.length/2)*S,(p.y+d.width/2-container.width/2)*S,(p.z+p.height/2)*S); o.rotation.set(0,0,p.rotation*Math.PI/180) }, [p.x,p.y,p.z,p.rotation,p.length,p.width,p.height,container.length,container.width])
...
function MaterialObject({ item, container, selected, onSelect, registerRef, onDelete }: { item: SecuringItem; container: Container; selected: boolean; onSelect: () => void; registerRef: (id: string, o: THREE.Group | null) => void; onDelete: () => void }) {
  ...
  const maxX=Math.max(0,container.length-item.length),maxY=Math.max(0,container.width-item.width),maxZ=Math.max(0,container.height-item.height),x=Math.max(0,Math.min(maxX,item.x)),y=Math.max(0,Math.min(maxY,item.y)),z=Math.max(0,Math.min(maxZ,item.z)),pos:[number,number,number]=[(x+item.length/2-container.length/2)*S,(y+item.width/2-container.width/2)*S,(z+item.height/2)*S]
...
function SceneContent({props}:{props:Props}){
 ...
 const [tool,setTool]=useState<SceneTool>('translate'),[sceneSelectionMode,setSceneSelectionMode]=useState<'single'|'multi'>(props.selectionMode==='box'?'multi':'single'),[freePlacementEnabled,setFreePlacementEnabled]=useState(!!props.freePlacement),[focusPoint,setFocusPoint]=useState<[number,number,number]|null>(null),[focusNonce,setFocusNonce]=useState(0)
 const activeMaterial=props.selectedMaterialId?materials.find(m=>m.id===props.selectedMaterialId)??null:null,activeId=activeMaterial?.id??(freePlacementEnabled?selectedId:null),activeObject=activeId?refs.current[activeId]??null:null,multi=sceneSelectionMode==='multi'
 const registerRef=useCallback((id:string,o:THREE.Group|null)=>{if(o)refs.current[id]=o;else delete refs.current[id]},[])
 const selectCargo=(id:string)=>{props.onSelectMaterial?.(null);if(freePlacementEnabled){const p=items.find(q=>q.id===id);if(p){const d=dims(p);setFocusPoint([(p.x+d.length/2-container.length/2)*S,(p.y+d.width/2-container.width/2)*S,(p.z+p.height/2)*S]);setFocusNonce(n=>n+1)}}if(multi){const next=selectedIds.includes(id)?selectedIds.filter(x=>x!==id):[...selectedIds,id];onSelectMany(next)}else onSelect(id)}
 ...
 const commit=()=>{
  if(activeMaterial){const o=refs.current[activeMaterial.id];if(!o)return;const r=((Math.round(o.rotation.z/(Math.PI/2))*90)%360+360)%360,rawX=Math.round((o.position.x/S-activeMaterial.length/2+container.length/2)/10)*10,rawY=Math.round((o.position.y/S-activeMaterial.width/2+container.width/2)/10)*10;onMaterialMove(activeMaterial.id,Math.max(0,Math.min(container.length-activeMaterial.length,rawX)),Math.max(0,Math.min(container.width-activeMaterial.width,rawY)));onRotateMaterial(activeMaterial.id,r);if(tool==='scale'){const s=snapshots.current[activeMaterial.id];if(s){const f=Math.max(.25,Math.min(4,o.scale.x/Math.max(.001,s.scale.x)));o.scale.set(1,1,1);onMaterialScale(activeMaterial.id,f)}}return}
 ...
 return <><ambientLight intensity={1.45}/><directionalLight position={[8,10,18]} intensity={1.8}/><hemisphereLight intensity={.35} groundColor="#dbe3e8" color="#fff"/><ContainerStructure container={container} lang={lang} showDimensions={showDimensions}/><ContainerFloor container={container}/><LashingPoints container={container}/><CoordinateAxes container={container}/><InstancedCartons items={instancedCartons} container={container} onSelect={selectCargo}/>{individualItems.map(p=><CargoObject key={p.id} p={p} container={container} selected={selectedIds.includes(p.id)||p.id===selectedId} onSelect={()=>selectCargo(p.id)} registerRef={registerRef} showName={!!cargo.find(c=>c.id===p.cargoId)?.showName}/>)}{materials.map(m=><MaterialObject key={m.id} item={m} container={container} selected={m.id===props.selectedMaterialId} onSelect={()=>{onSelect(null);onSelectMany([]);props.onSelectMaterial?.(m.id)}} onDelete={()=>{props.onDeleteMaterial?.(m.id);props.onSelectMaterial?.(null)}} registerRef={registerRef}/>)}{activeObject&&(activeMaterial||freePlacementEnabled)&&<TransformControls object={activeObject} mode={activeMaterial&&tool==='scale'?'scale':tool} axis={undefined} translationSnap={.01} rotationSnap={Math.PI/2} scaleSnap={.05} showX={tool==='rotate'?true:true} showY={tool==='rotate'?false:true} showZ onMouseDown={(event:any)=>{event.stopPropagation();capture();onDragState(true)}} onMouseUp={(event:any)=>{event.stopPropagation();commit();requestAnimationFrame(()=>onDragState(false))}} onChange={()=>{applyMulti();rememberPending()}}/>}<CameraRig view={view} container={container} dragging={dragging} transformActive={!!activeObject&&!!(activeMaterial||freePlacementEnabled)} focusPoint={focusPoint} focusNonce={focusNonce}/></>
}
